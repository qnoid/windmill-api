...
public class AuthorizationClaimContainerRequestFilter implements ContainerRequestFilter {

	public static final Logger LOGGER = Logger.getLogger(AuthorizationClaimContainerRequestFilter.class);
	...
	public void filter(ContainerRequestContext requestContext) throws IOException {
		...
		try {
    		JWT<JWS> jwt = Authorization.get(requestContext.getHeaderString(HttpHeaders.AUTHORIZATION));                    
			...
			if(claims.hasExpired()) {
				LOGGER.debug(String.format("Claim expired %s minutes ago at: %s", ChronoUnit.MINUTES.between(claims.exp, Instant.now()), claims.exp));				
				requestContext.abortWith(Response.status(Status.UNAUTHORIZED).build());
			}
    	}
